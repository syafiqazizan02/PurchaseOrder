				...
				<div class="row">
					...
					<div>
						<table>
							...
							<tr height="60">
							<td></td>
							<td ><?php echo "<a href='pro_display_item.php?username=$username&&res_id=$res_id'><input type='button' class='btn btn-success' value=' ADD ORDER '></a>";?>&nbsp;&nbsp;<?php echo "<a href='pro_display_item_pro.php?username=$username&&res_id=$res_id'><button type='submit'  name='edit' class='btn btn-primary'><span class='glyphicon glyphicon-edit'></span></button></a>";?></td>
							</tr>
						</table>
					</div>
				</div><br>
				...
				<div class="row">
					...
					<div class="col-lg-12">
						...
						<?php
							...
							$result = mysql_query("SELECT * from pro_item WHERE username='$b' AND res_id='$a' ");
							...
							while($row=mysql_fetch_array($result))
							{
								...
								echo "<td align=center>
										<a href='pro_edit_item.php?id=".($row["item_id"])."&&res_id=".($row['res_id'])."&&username=$username'>
											<button type='submit'  name='edit' class='btn btn-success btn-xs'><span class='glyphicon glyphicon-edit'></span></button></a>
										<a href='pro_delete_item.php?id=".($row["item_id"])."&&res_id=".($row['res_id'])."&&username=$username'>
							...
							$insert_pro=mysql_query("UPDATE pro_request SET total='$total' where res_id='$a'");
