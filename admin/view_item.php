				...
				<div class="row">
					...
					<div>
						<table>
							...
							<tr height="60">
							<td></td>
							<td ><label><?php echo "<a href='display_item.php?res_id=$res_id'><input type='button' class='btn btn-success' value=' ADD ORDER '></a>";?></label></td>
							</tr>
						</table>
					</div>
				</div><br>
						...
						<?php
							...
							while($row=mysql_fetch_array($result))
							{
								...
								echo "<td align=center>
										<a href='edit_item.php?id=".($row["item_id"])." & res_id=".($row['res_id'])."'>
											<button type='submit'  name='edit' class='btn btn-success btn-xs'><span class='glyphicon glyphicon-edit'></span></button></a>
										<a href='delete_item.php?id=".($row["item_id"])." & res_id=".($row['res_id'])."'>
