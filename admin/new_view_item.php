				...
				<div class="row">
					...
					<div>
						<table>
							...
							<tr height="60">
							<td></td>
							<td ><label><?php echo "<a href='new_display_item.php?res_id=$res_id'><input type='button' class='btn btn-success' value=' ADD ORDER '></a>";?></label></td>
							</tr>
						</table>
					</div>
				</div><br>
				...
				<div class="row">
					...
					<div class="col-lg-12">
						...
						<?php
							...
							$result = mysql_query("SELECT * from new_item WHERE res_id='$a' ");
							...
							while($row=mysql_fetch_array($result))
							{
								...
								echo "<td align=center>
										<a href='new_edit_item.php?id=".($row["item_id"])." & res_id=".($row['res_id'])."'>
											<button type='submit'  name='edit' class='btn btn-success btn-xs'><span class='glyphicon glyphicon-edit'></span></button></a>
										<a href='new_delete_item.php?id=".($row["item_id"])." & res_id=".($row['res_id'])."'>
